...
# Configure Django
os.environ.setdefault("DJANGO_SETTINGS_MODULE", "mousetube_api.settings")  # <-- Remplace par ton projet
django.setup()

from mousetube_api.models import Contact, User, Software  # <-- Remplace "mousetube_api" par ton nom d'application
...
def migrate_contacts_to_users(dry_run=True):
    ...
    for contact in Contact.objects.all():
        ...
        user = User.objects.filter(name_user=contact.lastname , email_user=contact.email).first()
    ...
    for sw, usr, ct in linked_softwares:
        print(f"- Software '{sw.software_name}' lié à User '{usr.name_user}' (depuis Contact '{ct.lastname}')")
